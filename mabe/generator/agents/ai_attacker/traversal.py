...
5. Neighbor enumeration — for each ACL-permitted neighbor reachable under
   ...
   a. Single auth attempt using the node's first supported protocol
   b. On failure: node is marked unreachable for this session
   c. On success: node is enqueued and credential harvest is rolled
...
class BFSTraversalAgent:
    ...
    def _attempt_neighbor_auth(
        ...
        """
        Attempt authentication against a neighbor node using the node's
        first supported protocol. Returns True if authentication succeeded
        and the node should be enqueued for BFS traversal.
        """
        node_type = dst_attrs.get("node_type", "workstation")
        protocol = dst_attrs.get("auth_protocols", ["ntlm"])[0]
        ...
        # Auth succeeded — roll credential harvest
        if self._rng.random() < self._harvest_probability:
            credential = self._sample_credential(
                node_id=dst_node,
                node_type=node_type,
            )
    ...
    def _attempt_auth(
        ...
        """
        Single auth attempt used for file server node-level authentication
        before file access events are generated. Returns True on success.
        """
        src_attrs = self._graph.nodes[src_node]
        protocol = node_attrs.get("auth_protocols", ["ntlm"])[0]
    ...
    def _sample_credential(self, node_id: str, node_type: str) -> Credential:
        """
        Sample a credential privilege level for the given node type and
        return a new Credential object.

        Privilege level is drawn from the weight distribution configured
        for the node type in behavioral_params.yaml. Domain controllers and
        logging infrastructure yield high-privilege credentials far more
        often than workstations.
        """
        weights = self._params.get(
    ...
    def _emit_file_access(
        ...
        if self._rng.random() < self._file_harvest_prob:
